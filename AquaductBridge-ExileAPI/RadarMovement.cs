...
namespace RadarMovement
{
    ...
    public class RadarMovement : BaseSettingsPlugin<RadarMovementSettings>
    {
        ...
        public override bool Initialise()
        {
            try
            {
                // Initialize advanced mouse controller with anti-kick measures
                AdvancedMouse.Initialize(GameController);
                
                // Initialize enhanced LineOfSight system
                lineOfSight = new LineOfSight(GameController);
                
                // Initialize hybrid pathfinding system (Radar + A*)
                ...
                AddDebugMessage("RadarMovement Hybrid v5.0 initialized with AdvancedMouse, Enhanced LineOfSight, Direction Fields, and Radar-style visualization");
                LogMessage("RadarMovement Hybrid v5.0 - Combining Radar visualization with Follower movement capabilities and AreWeThereYet terrain analysis");
        ...
        private IEnumerator MainProcessingCoroutine()
        {
            while (true)
            {
                ...
                {
                    LogError($"Error updating performance stats: {ex.Message}");
                }
                ...
                {
                    LogError($"Error processing task queue: {ex.Message}");
                    AddDebugMessage($"Task queue error: {ex.Message}");
                }
        ...
        private IEnumerator PostAreaTransitionGracePeriod()
        {
            ...
            while (stopwatch.ElapsedMilliseconds < timeoutMs)
            {
                ...
                var currentAreaName = "Unknown";
                var gameStable = false;
                
                try
                {
                    currentAreaName = GameController.Area?.CurrentArea?.DisplayName ?? "Unknown";
                    gameStable = GameController.InGame && GameController.Player != null && !GameController.IsLoading;
                }
                catch (Exception ex)
                {
                    LogError($"Error checking game state in grace period: {ex.Message}");
                }
        ...
        private void ScanForNewTasks()
        {
            ...
            foreach (var entity in entities)
            {
                ...
                var targetWorld = new Vector3(entity.GridPos.X, entity.GridPos.Y, 0f);
                ...
                if (pathStatus == PathStatus.Blocked && pathfinderInitialized && 
                    ShouldUsePathfinding(new Vector3(playerPos.X, playerPos.Y, 0f), targetWorld))
                {
        ...
        private async void MoveTowardsTask(RadarTask task, Vector2 playerPos)
        {
            try
            {
                var targetWorldPos = new Vector2(task.WorldPosition.X, task.WorldPosition.Y);
                
                // Use enhanced path status checking with dash detection
                var pathStatus = lineOfSight?.GetPathStatus(playerPos, targetWorldPos) ?? PathStatus.Clear;
                ...
                else if (pathStatus == PathStatus.Dashable && Settings.Movement.UseDashForObstacles.Value)
                {
                    // Path is dashable - we could use dash skill here
                    AddDebugMessage($"Dashable path detected to {task.Type}");
                }

                // Use AdvancedMouse for human-like movement with anti-kick measures
                var targetPos3D = new SharpDX.Vector3(actualTarget.X, actualTarget.Y, 0);
                var safeScreenPos = AdvancedMouse.WorldToValidScreenPosition(targetPos3D);

                if (safeScreenPos != Vector2.Zero)
                {
                    // Use human-like cursor movement and clicking
                    await AdvancedMouse.SetCursorPosAndLeftClickHuman(safeScreenPos, 50);
                    ...
                    var mouseInfo = AdvancedMouse.GetActionRateInfo();
                    AddDebugMessage($"Moving to {task.Type} at distance {task.GetDistanceFrom(new SharpDX.Vector2(playerPos.X, playerPos.Y)):F1}{statusInfo} | {mouseInfo}");
                }
                else
                {
                    AddDebugMessage($"Could not find safe screen position for target");
                    task.RecordAttempt();
                }
            }
            catch (Exception ex)
        ...
        private void RenderVisuals()
        {
            ...
            try
            {
                var playerPos = new Vector2(GameController.Player.Pos.X, GameController.Player.Pos.Y);

                // Enhanced task visualization with Radar-style path drawing
                if (Settings.Visual.ShowPathLine.Value && currentTask != null)
                {
                    DrawEnhancedTaskLine(currentTask);
                }

                // Radar-style path flow visualization 
                if (Settings.Pathfinding.ShowDirectionFields.Value && pathfinderInitialized)
                {
                ...
                {
                    DrawEnhancedTaskQueue();
                }

                if (Settings.Visual.ShowTargetMarker.Value && currentTask != null)
                {
                    DrawEnhancedTargetMarker(currentTask);
                }

                // Advanced terrain analysis visualization with path status
                if (Settings.Debug.ShowPathRays.Value && lineOfSight != null)
                {
                    DrawEnhancedLineOfSightRays();
                }

                if (Settings.Debug.ShowTerrain.Value)
                {
                    DrawAdvancedTerrainAnalysis(playerPos);
                }

                // Pathfinding visualization with multiple path options
                if (Settings.Pathfinding.ShowPathLines.Value && pathfinderInitialized)
                {
                    DrawAdvancedPathfindingVisualization();
                }

                if (Settings.Pathfinding.ShowWaypoints.Value && pathfinderInitialized)
                {
                    DrawEnhancedWaypoints();
                }

                // Mouse action rate visualization
                if (Settings.Debug.ShowMouseInfo.Value)
                {
                    DrawMouseActionInfo();
                }

                // Pathfinder performance stats
                if (Settings.Debug.ShowPathfindingStats.Value && pathfinderInitialized)
                {
        ...
        #region Enhanced Visualization Methods (Hybrid Features)

        private void DrawEnhancedTaskLine(RadarTask task)
        {
            var camera = GameController.IngameState.Camera;
            var playerPos = GameController.Player.GridPos;
            var playerPos3D = new Vector3(playerPos.X, playerPos.Y, 0);
            ...
            {
                var lineColor = GetTaskColor(task.Type);
                var lineWidth = Settings.Visual.PathLineWidth.Value;
                
                // Draw animated line (Radar-style)
                var animationOffset = (DateTime.Now.Millisecond / 100.0f) % 1.0f;
                
                // Draw main path line
                Graphics.DrawLine(playerScreen, targetScreen, lineWidth, lineColor);
                
                // Draw movement indicators (small circles along the path)
                var distance = Vector2.Distance(playerScreen, targetScreen);
                var segments = Math.Max(2, (int)(distance / 50));
                
                for (int i = 1; i < segments; i++)
                {
                    var t = (i / (float)segments + animationOffset) % 1.0f;
                    var pos = Vector2.Lerp(playerScreen, targetScreen, t);
                    var size = 4 + (float)Math.Sin(DateTime.Now.Millisecond / 200.0) * 2;
                    Graphics.DrawEllipse(pos, new Vector2(size * 2, size * 2), lineColor, 1);
                }
                
                // Draw distance text
                var midPoint = Vector2.Lerp(playerScreen, targetScreen, 0.5f);
                var distanceText = $"{task.GetDistanceFrom(new SharpDX.Vector2(playerPos.X, playerPos.Y)):F0}";
                Graphics.DrawText(distanceText, midPoint + new Vector2(10, -10), Color.White);
            }
        }

        private void DrawEnhancedTargetMarker(RadarTask task)
        {
            var camera = GameController.IngameState.Camera;
            var targetPos3D = new Vector3(task.WorldPosition.X, task.WorldPosition.Y, 0);
            var targetScreen = camera.WorldToScreen(targetPos3D);
            ...
            if (IsValidScreenPosition(targetScreen))
            {
                ...
                var pulseRadius = 12f + (float)Math.Sin(time * 6) * 4f; // Pulsing effect
                
                // Draw pulsing circle
                Graphics.DrawEllipse(targetScreen, new Vector2(pulseRadius * 2, pulseRadius * 2), color, 2);
                
                // Draw directional arrow pointing to target
                var playerPos = GameController.Player.GridPos;
                var direction = Vector2.Normalize(new Vector2(task.WorldPosition.X - playerPos.X, task.WorldPosition.Y - playerPos.Y));
                var arrowEnd = targetScreen - direction * pulseRadius;
                var arrowStart = arrowEnd - direction * 20;
                
                Graphics.DrawLine(arrowStart, arrowEnd, 3, color);
                
                // Draw task type icon/text
                var taskText = task.Type.ToString().Substring(5); // Remove "Click" prefix
                Graphics.DrawText(taskText, targetScreen + new Vector2(15, -8), color);
            }
        }

        private void DrawEnhancedTaskQueue()
        {
            var startY = 200;
            var lineHeight = 18;
            var x = 10;
            ...
            // Background panel
            var panelRect = new RectangleF(x - 5, startY - 5, 300, (taskQueue.Count + 3) * lineHeight + 10);
            Graphics.DrawBox(panelRect, Color.Black.ToAlpha(180));
            Graphics.DrawFrame(panelRect, Color.Cyan, 1);

            Graphics.DrawText($"Task Queue ({taskQueue.Count}) - Hybrid v5.0", new Vector2(x, startY), Color.Cyan);
            startY += lineHeight;

            if (currentTask != null)
            {
                var currentColor = Color.Yellow;
                var status = currentTask.IsCompleted ? "[DONE]" : $"[{currentTask.AttemptCount}/3]";
                Graphics.DrawText($"CURRENT: {currentTask} {status}", new Vector2(x, startY), currentColor);
                startY += lineHeight;
            }
            ...
            for (int i = 0; i < displayCount; i++)
            {
                var task = taskArray[i];
                var taskColor = GetTaskColor(task.Type);
                var priority = task.Priority > 0 ? $" (P{task.Priority})" : "";
                Graphics.DrawText($"{i + 1}: {task}{priority}", new Vector2(x, startY), taskColor);
                startY += lineHeight;
            }
        ...
        private void DrawDirectionFieldVisualization(Vector2 playerPos)
        {
            if (pathfinder == null || currentTask == null) return;

            try
            {
                var camera = GameController.IngameState.Camera;
                var targetGrid = AqueductPathfinder.WorldToGrid(new Vector3(currentTask.WorldPosition.X, currentTask.WorldPosition.Y, 0));
                
                // Draw direction field arrows in a grid around the player
                var playerGrid = AqueductPathfinder.WorldToGrid(new Vector3(playerPos.X, playerPos.Y, 0));
                var range = 20; // Grid units around player
                
                for (int dx = -range; dx <= range; dx += 5) // Sample every 5th grid point for performance
                {
                    for (int dy = -range; dy <= range; dy += 5)
                    {
                        var gridPos = new Vector2i(playerGrid.X + dx, playerGrid.Y + dy);
                        var worldPos = AqueductPathfinder.GridToWorld(gridPos);
                        var screenPos = camera.WorldToScreen(new SharpDX.Vector3(worldPos.X, worldPos.Y, worldPos.Z));
                        
                        if (IsValidScreenPosition(screenPos))
                        {
                            // This would require access to the pathfinder's direction field
                            // For now, draw a simplified version showing flow towards target
                            var toTarget = Vector2.Normalize(new Vector2(currentTask.WorldPosition.X - worldPos.X, currentTask.WorldPosition.Y - worldPos.Y));
                            var arrowEnd = screenPos + toTarget * 10;
                            
                            Graphics.DrawLine(screenPos, arrowEnd, 1, Color.Green.ToAlpha(100));
                            
                            // Draw arrow head
                            var perpendicular = new Vector2(-toTarget.Y, toTarget.X);
                            Graphics.DrawLine(arrowEnd, arrowEnd - toTarget * 5 + perpendicular * 3, 1, Color.Green.ToAlpha(100));
                            Graphics.DrawLine(arrowEnd, arrowEnd - toTarget * 5 - perpendicular * 3, 1, Color.Green.ToAlpha(100));
                        }
                    }
                }
            ...
            {
                AddDebugMessage($"Direction field visualization error: {ex.Message}");
            }
        }

        private void DrawEnhancedLineOfSightRays()
        {
            // LineOfSight system now handles its own debug visualization through events
            // This method could add additional overlay information
            if (currentTask != null)
            {
                var playerPos = new Vector2(GameController.Player.Pos.X, GameController.Player.Pos.Y);
                var targetPos = new Vector2(currentTask.WorldPosition.X, currentTask.WorldPosition.Y);
                var pathStatus = lineOfSight?.GetPathStatus(playerPos, targetPos) ?? PathStatus.Clear;
                
                // Draw path status indicator
                var camera = GameController.IngameState.Camera;
                var playerScreen = camera.WorldToScreen(new SharpDX.Vector3(playerPos.X, playerPos.Y, 0));
                ...
                Graphics.DrawText($"Path: {pathStatus}", playerScreen + new Vector2(20, -40), statusColor);
            }
        }

        private void DrawAdvancedTerrainAnalysis(Vector2 playerPos)
        {
            if (lineOfSight == null) return;

            try
            {
                var analysis = lineOfSight.AnalyzeArea();
                var startY = 400;
                var x = 10;
                var lineHeight = 16;
                
                // Background panel
                var panelRect = new RectangleF(x - 5, startY - 5, 250, analysis.Count * lineHeight + 20);
                Graphics.DrawBox(panelRect, Color.Black.ToAlpha(160));
                Graphics.DrawFrame(panelRect, Color.Cyan, 1);
                ...
                {
                    var color = kvp.Key == "Error" ? Color.Red : Color.White;
                    var text = kvp.Value is float f ? $"{kvp.Key}: {f:F1}" : $"{kvp.Key}: {kvp.Value}";
                    Graphics.DrawText(text, new Vector2(x, startY), color);
                    startY += lineHeight;
                }
                
                // Show terrain info at player position
                var terrainInfo = lineOfSight.GetTerrainInfo(playerPos);
                Graphics.DrawText($"Player Position: {terrainInfo}", new Vector2(x, startY), Color.LightGreen);
            }
            catch (Exception ex)
            {
                AddDebugMessage($"Terrain analysis error: {ex.Message}");
            }
        }

        private void DrawAdvancedPathfindingVisualization()
        {
            // This could show multiple path options, waypoint connections, etc.
            if (pathfinder == null || currentTask == null) return;
            ...
            try
            {
                ...
                foreach (var waypoint in waypoints)
                {
                    var waypointScreen = camera.WorldToScreen(new SharpDX.Vector3(waypoint.WorldPosition.X, waypoint.WorldPosition.Y, waypoint.WorldPosition.Z));
                    
                    if (IsValidScreenPosition(waypointScreen))
                    {
                        foreach (var connected in waypoint.ConnectedWaypoints)
                        {
                            var connectedScreen = camera.WorldToScreen(new SharpDX.Vector3(connected.WorldPosition.X, connected.WorldPosition.Y, connected.WorldPosition.Z));
                            if (IsValidScreenPosition(connectedScreen))
                            {
                                var color = waypoint.Type switch
                                {
                                    TerrainType.Bridge => Color.Blue,
                                    TerrainType.Exit => Color.Red,
                                    _ => Color.Gray
                                };
                                Graphics.DrawLine(waypointScreen, connectedScreen, 1, color.ToAlpha(100));
                            }
                        }
                    }
                }
            ...
            {
                AddDebugMessage($"Pathfinding visualization error: {ex.Message}");
            }
        }

        private void DrawEnhancedWaypoints()
        {
            if (pathfinder == null) return;
            ...
            try
            {
                ...
                foreach (var waypoint in waypoints)
                {
                    ...
                    if (IsValidScreenPosition(waypointScreen))
                    {
                        var color = waypoint.Type switch
                        {
                            TerrainType.Bridge => Color.CornflowerBlue,
                            TerrainType.Exit => Color.Red,
                            TerrainType.Waypoint => Color.Yellow,
                            _ => Color.White
                        };
                        
                        var size = waypoint.Priority * 8 + 6; // Size based on priority
                        Graphics.DrawEllipse(waypointScreen, new Vector2(size, size), color, 2);
                        Graphics.DrawText(waypoint.Name, waypointScreen + new Vector2(10, -5), color);
                    }
                }
            }
            catch (Exception ex)
            {
                AddDebugMessage($"Waypoint visualization error: {ex.Message}");
            }
        }

        private void DrawMouseActionInfo()
        {
            var mouseInfo = AdvancedMouse.GetActionRateInfo();
            var startY = 100;
            var x = 10;
            ...
            // Background
            var textSize = Graphics.MeasureText(mouseInfo);
            var panelRect = new RectangleF(x - 5, startY - 5, textSize.X + 10, textSize.Y + 10);
            Graphics.DrawBox(panelRect, Color.Black.ToAlpha(150));
            Graphics.DrawFrame(panelRect, Color.Green, 1);
            
            var color = mouseInfo.Contains("LIMITED") ? Color.Red : Color.Green;
            Graphics.DrawText($"Mouse: {mouseInfo}", new Vector2(x, startY), color);
        }

        private void DrawPathfindingPerformanceStats()
        {
            if (pathfinder == null) return;
            
            var stats = pathfinder.GetPerformanceStats();
            var startY = 130;
            var x = 10;
            
            // Background
            var textSize = Graphics.MeasureText(stats);
            var panelRect = new RectangleF(x - 5, startY - 5, textSize.X + 10, textSize.Y + 10);
            Graphics.DrawBox(panelRect, Color.Black.ToAlpha(150));
            Graphics.DrawFrame(panelRect, Color.Blue, 1);
            
            Graphics.DrawText($"Pathfinder: {stats}", new Vector2(x, startY), Color.Cyan);
        }
